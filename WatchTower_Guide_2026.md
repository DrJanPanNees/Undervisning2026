...
## Trin 5: GitHub Actions – automatisk build & push til GHCR

**Formål:** Når du pusher til `main`, skal GitHub Actions automatisk bygge et Docker-image og uploade det til GHCR. Det gør det muligt for WatchTower at opdage og opdatere containeren.

### Sådan gør du:

1. **Opret workflow-mappen og filen:**
```sh
mkdir -p .github/workflows
```

2. **Opret `.github/workflows/docker-publish.yml` med dette indhold:**
```yaml
  ...
  build:
    ...
    steps:
      ...
      - uses: docker/login-action@v3
        with:
          registry: ghcr.io
          username: ${{ github.actor }}         # <-- Bruges automatisk, ingen ændring nødvendig
          password: ${{ secrets.GITHUB_TOKEN }} # <-- Bruges automatisk, ingen ændring nødvendig
      - name: Build and push
        ...
        with:
          ...
          # Opdater 'helloworld' til dit eget image-navn, hvis du har valgt et andet
```

3. **Commit og push workflowet:**
```sh
git add .
git commit -m "Tilføj GitHub Actions workflow"
git branch -M main
git push -u origin main
```

4. **Tjek at workflowet kører:**
- Gå til dit repo på GitHub → Actions.
- Du skal se, at workflowet kører grønt.
- Dit image findes under “Packages” i repoet.

### Fejl og løsninger
- **Fejl:** `refusing to allow a Personal Access Token to create or update workflow ... without workflow scope`
  - **Løsning:** Brug SSH (se guide nedenfor) eller lav en PAT med både `repo` og `workflow` scopes.
- **Fejl:** `Workflow kører ikke`
  - **Løsning:** Tjek at du pusher til `main`-branch, og at workflow-filen ligger i `.github/workflows/`.
- **Fejl:** `Cannot find Dockerfile`
  - **Løsning:** Sørg for at stien i workflow-filen matcher placeringen af din Dockerfile.


## Trin 6: Verificér WatchTower med en kodeændring

**Formål:** Du skal nu se, at WatchTower automatisk opdaterer containeren, når du ændrer koden og pusher.

### Sådan gør du:

1. **Lav en lille ændring i din kode** (fx i `Program.cs`):
```csharp
...
```sh
git add Program.cs
git commit -m "Ændr output til Hello, WatchTower!"
git push
```

3. **Vent til GitHub Actions-workflowet er færdigt** (tjek under Actions på GitHub).

4. **WatchTower vil nu automatisk opdage det nye image og genstarte containeren.**

5. **Tjek logs på din server:**
```sh
docker logs <helloworld-container>
```
Du bør nu se den nye tekst – WatchTower har automatisk opdateret containeren!

### Visualisering af flowet
```text
[Push til main] → [GitHub Actions bygger & pusher image til GHCR] → [WatchTower poller] → [Container opdateres automatisk]
```


 Verificér WatchTower med en kodeændring (ingen Compose-ændring)
